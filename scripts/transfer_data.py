...
def transfer(maples: MapleAPI, mapled: MapleAPI, n: int):
    '''transfer articles from one backend to another'''
    transferred = 0
    attempted = 0
    while True:
        for articles in maples.article_iterator(limit=1, page=attempted):
            if len(articles) == 0:
                return
...
if __name__ == '__main__':
    ...
    try:
        ...
        transfer(maples=maples, mapled=mapled, n = args.n)
