...
class CBCArticleExtractor:
    ...
    def extract_author(response, data: dict = None):
        ...
        for i, authorselector in enumerate(
            selector.xpath('//div[@class="authorprofile-container"]')
        ):
            if i > 0:
                author = Author()
            # set name
            if getattr(author, "name", "") == "":
                author.name = authorselector.xpath(
                    '//div[@class="authorprofile-name-container"]//a//text()'
                ).get()
            # set url
            if getattr(author, "url", "") == "":
                try:
                    url = authorselector.xpath(
                        '//div[@class="authorprofile-name-container"]/a/@href'
                    ).get()
                    if url:
            ...
            if getattr(author, "title", "") == "":
                title = authorselector.xpath(
                    '//p[@class="authorprofile-title"]//text()'
                ).get()
            ...
            if getattr(author, "about", "") == "":
                try:
                    about = authorselector.xpath(
                        '//p[@class="authorprofile-biography"]//text()'
                    ).get()
                    author.about = about
                    email = None
                    try:
                        ...
                        logging.debug(
                            f"extract_author:Could not set email {email}. {e}"
                        )
                except Exception as e:
                    logger.debug(
                        f"extract_author:Could not retrieve author biography. {e}"
                    )
    ...
    def from_response(response, extra: dict = {}):
        ...
        if "title" in extra.keys():
            ...
            article.title = response.xpath(
                '//h1[@class="detailHeadline"]//text()'
            ).get()

        article.url = response.url

        article.content = "\n".join(
            response.xpath('//div[@class="story"]//text()').getall()
        )
        ...
        if "publishTime" in extra.keys():
            try:
                ...
                logger.debug(
                    f"from_response:Could not convert datetime from {extra['publishTime']}. {e}"
                )
        ...
        if date_published is not None:
            try:
                ...
                logger.debug(
                    f"from_response:Could not set date_published from {date_published}. {e}"
                )
        ...
        if "updateTime" in extra.keys():
            try:
                ...
                logger.debug(
                    f"from_response: could not fetch modified date from {extra['updateTime']}. {e}"
                )
        if date_modified is not None:
            try:
                ...
                logger.debug(
                    f"from_response:Could not set article field date_modified from: {date_modified}. {e}"
                )
...
class CBCHeadlinesSpider(scrapy.Spider):
    ...
    def __init__(self, name=None, **kwargs):
        ...
        location_links = [
            str.replace(re.sub("[ˆ\W]\s", "", location), " ", "-").lower()
    ...
    def parse_news_content(self, response):
        ...
        article = CBCArticleExtractor.from_response(
            response=response, extra=self.url_mapping[response.url]
        )
