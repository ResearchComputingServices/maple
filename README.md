...
- [Youtube](#youtube---tool-developed-by-rcs)
- [Existing News APIs](#apis---investigation-of-third-party-tools-to-query-news)
- [Spiders + standardized output](#custom-spiders---rcs-implementation)
...
Spiders can follow news links from a list of websites. Each website has its own implementation and therefore a new spider is required. 

### CBC

At this moment, we implemented the CBC spider to go over news based on localities, and all rss feeds available in [CBC RSS](http://www.cbc.ca/rss). In addition, the data is exported in json with standard keys.
...
A huge amount of data can be extracted from the already formated json file provided above. However, it would be interesting to standardize the output from multiple sources. To achieve this goal, we propose the (not finalized) structure as below ([CBC spider output with proposed output can be found here](/sample/sample_proposed_output.json)):
...
- Mapping output of different APIs to our standardized output.
- Retrieve comments for news
- Sample data for CBC is not fully implemented, but gives an idea about capabilities
- Load more data (dynamically loaded content is not currently implemented)

### Drawbacks
- The spiders should be constantly maintained
    - If the structure of the website changes, so does the crawlers.
- No queries most of the time
    - From what we have seen, most News websites do not allow queries.
    - Queries could be implemented on the downloaded data
